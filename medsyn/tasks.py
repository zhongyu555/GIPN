...
class BaseTask(ABC):
    ...
    def apply(self,
        ...
        sample_shape = np.array(sample.shape[1:])  # 提取样本的形状（去掉批次维度），用于计算异常区域的大小
        anomaly_mask = np.zeros(sample_shape, dtype=bool)

        min_anom_prop = self.min_anom_prop  # 0.3
        max_anom_prop = self.max_anom_prop  # 0.8

        min_dim_lens = (min_anom_prop * sample_shape).round().astype(int)  # [67,67]
        max_dim_lens = (max_anom_prop * sample_shape).round().astype(int)  # [179,179]
        ...
        sample_mask = None  # 样本的对象掩码（如果有），表示哪些区域属于样本的感兴趣部分，类型为布尔数组

        for i in range(2):

            # Compute anomaly mask   创建一个异常区域的掩码（curr_anomaly_mask）及其交集函数（intersect_fn）
            curr_anomaly_mask, intersect_fn = self.anomaly_shape_maker.get_patch_mask_and_intersect_fn(dim_bounds,
                                                                                                        sample_shape)
            #  curr_anomaly_mask [159,81]     intersect_fn None

            # Choose anomaly location
            anomaly_corner = self.find_valid_anomaly_location(curr_anomaly_mask, sample_mask, sample_shape)  #是一个坐标 [43,101]

            # Apply self-supervised task
            # 这个类作为父类的，是GaussIntensityChangeTask类继承了这个父类（假设采用的高斯合成），所以会在这里调用一下父类 ，在GaussIntensityChangeTask类有augment_sample函数的具体实现方法
            aug_sample = self.augment_sample(aug_sample, sample_mask, anomaly_corner, curr_anomaly_mask, intersect_fn)    # aug_sample [1,224,224]
            # aug_sample 待增强的样本,  sample_mask 样本的对象掩码（如果有）,  anomaly_corner 异常区域的起始位置（corner index），即异常区域在样本中的左上角坐标 ,
            # curr_anomaly_mask 布尔掩码，用于定义异常区域的形状, intersect_fn 一个函数，定义如何计算线段与异常区域边界的交点，用于辅助生成异常效果。

            anomaly_mask[get_patch_slices(anomaly_corner, curr_anomaly_mask.shape)] |= curr_anomaly_mask  #  anomaly_mask[224,224]
    ...
    # 如果 __call__ 函数定义在父类中，而子类没有对其进行重写（override），当子类的实例被直接调用时，会调用父类的 __call__ 方法
    def __call__(self,
                 sample: npt.NDArray[float],
                 *args,
                 **kwargs)\
        ...
        if len(sample.shape)==2:
            sample = np.expand_dims(sample,axis=0)  # [224,224] -> [1,224,224]

        aug_sample, aug_mask = self.apply(sample, *args, **kwargs)  # aug_sample [1,224,224], aug_mask [224,224]

        if len(aug_sample.shape)==3 and aug_sample.shape[0]==1:
            aug_sample = aug_sample.squeeze(0) # aug_sample恢复  [1,224,224]-> [224,224]
...
class GaussIntensityChangeTask(BaseTask):  # 通过高斯模糊和自定义纹理来生成和应用异常（anomaly）模式

    def __init__(self,
                 sample_labeller: Optional[AnomalyLabeller] = None,  # 用于标注样本的对象
                 **all_kwargs):

        super().__init__(sample_labeller, **all_kwargs)
        self.max_anom_prop = 0.8  # 控制异常比例范围
        self.min_anom_prop = 0.3
        self.sigma_bs = [4, 7]  # 用于高斯滤波器的标准差值候选列表
        self.positive_range = [0.4, 0.6]   #纹理的值范围，分别表示正值和负值
        self.negative_range = [-0.6, -0.4]
        self.anomaly_shape_maker = PerlinPatchMaker()  # 用于生成异常形状的对象，这里是 PerlinPatchMaker()
    def get_predefined_texture(self,           #  生成一个二维纹理，用于模拟异常模式
                               mask_shape,
                               sigma_b,
                               positive_range=None,
                               negative_range=None,
    ...
    def create_texture(self,sizes):   # 调用 get_predefined_texture 生成纹理
    ...
    def augment_sample(self,    # 在原始样本上应用异常模式，生成异常增强样本
                       sample: npt.NDArray[float], # aug_sample
                       sample_mask: Optional[npt.NDArray[bool]],# None
                       anomaly_corner: npt.NDArray[int], # anomaly center
                       anomaly_mask: npt.NDArray[bool], # small anomaly mask
